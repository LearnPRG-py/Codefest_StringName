...
public class getAIworking : MonoBehaviour
{
    ...
    private async Task<List<float>> FetchVectorsFromDataset(string datasetId)
    {
        using (HttpClient client = new HttpClient())
        {
            client.DefaultRequestHeaders.Add("Authorization", $"Bearer {apikey}");
            client.DefaultRequestHeaders.Add("OpenAI-Beta", "assistants=v2");
            string url = $"https://api.openai.com/v1/vector_stores/{datasetId}/vectors";
            ...
            // Assuming the response contains the vector data directly
            var vectorsData = JsonConvert.DeserializeObject<Dictionary<string, object>>(responseBody);
            if (vectorsData.ContainsKey("data"))
            {
                var vectors = JsonConvert.DeserializeObject<List<float>>(vectorsData["data"].ToString());
                Debug.Log("Deserialization Worked");
                return vectors;
            }
            else
            {
    ...
    private async Task GetGPTResponse()
    {
        using (HttpClient client = new HttpClient())
        {
            client.DefaultRequestHeaders.Add("Authorization", $"Bearer {apikey}");
            client.DefaultRequestHeaders.Add("OpenAI-Beta", "assistants=v2");
            var vectorData = await FetchVectorsFromDataset("vs_2yBw8EoMBYAw4PN9OFVwvCpV");
            var requestBody = new
            {
